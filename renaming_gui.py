...
class RenamingGUI:
    ...
    def _on_enter_key(self, event):
        ...
        # Prevent default Enter behavior (which deletes selected text)
        ...
        if isinstance(focused_widget, tk.Entry) and focused_widget == self.identifier_entry:
            # Apply current rename and move to next slide
            self._apply_current_rename()
            return "break"  # Prevent default Enter behavior (text deletion)
    
    def _highlight_identifier_text(self):
    ...
    def _perform_text_selection(self):
        """Perform the actual text selection."""
        try:
            if hasattr(self, 'identifier_entry') and self.identifier_entry:
                self.identifier_entry.focus_set()  # Set focus to the entry
                self.identifier_entry.select_range(0, tk.END)  # Select all text
